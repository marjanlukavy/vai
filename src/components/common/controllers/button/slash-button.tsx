  ...
  return (
    <div className={`relative w-full ${containerStyles}`}>
      <button
        type={type}
        onClick={onClick}
        className={`relative z-50 px-[30px] transition-all duration-300 ease-in-out rounded-[12px] hover:rounded-[16px] bg-[#77A9E829] backdrop-blur-[10px] main-shadow flex items-center gap-2 ${className}`}
      >
        {showIcon && <SlashIcon />}
        <span
